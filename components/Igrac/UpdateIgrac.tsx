...
const UpdateIgrac = (props: Props) => {
    ...
    const [nameInput, setNameInput] = useState(props.igrac.ime);
    const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) =>
        setNameInput(e.target.value);

    const [surnameInput, setSurnameInput] = useState(props.igrac.prezime);
    const handleSurnameChange = (e: React.ChangeEvent<HTMLInputElement>) =>
        setSurnameInput(e.target.value);

    const [eloInput, setEloInput] = useState(props.igrac.elo);
    const handleEloChange = (s: string, v: number) => setEloInput(v);

    const [locInput, setLocInput] = useState(
        props.locations.filter((v) => v.naziv == props.igrac.lokacija)[0].id
    );
    const handleLocChange = (e: React.ChangeEvent<HTMLSelectElement>) =>
        setLocInput(parseInt(e.target.value));

    const [titleInput, setTitleInput] = useState(props.igrac.titula_fide);
    const handleTitleChange = (e: React.ChangeEvent<HTMLSelectElement>) =>
        setTitleInput(e.target.value);

    const [imageInput, setImageInput] = useState(props.igrac.sahista_slika);
                            ...
                            onClick={async () => {
                                try {
                                    let res = await axios.put(
                                        `http://localhost:1207/sahista/${props.igrac.sahista_id}`,
                                        {
